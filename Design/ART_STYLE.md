...
- Тайлы: **64×64 px**, PPU = ширина тайла, point-фильтр. Пол/стены — **бесшовные**.
- Клетка мира `CellSize = 1` юнит; нахлёст тайлов `TileOverlap = 1.02`.
- **Стена = один плоский тайл** (`wall_top`, вид строго сверху). Игра — чистый
  2D top-down (камера `FramePerspective.CameraPitchDegrees = 0`), у стены НЕТ
  геометрической высоты — ничего за стеной не перекрывается. Иллюзию объёма дают
  текстуры и пропы (стиль The Escapists). Объём стены даёт САМА ТЕКСТУРА:
  бетонные блоки в перевязку со светлой верх-левой фаской и тёмной низ-правой +
  тёмный шов — каждый блок читается как приподнятый куб (свет сверху-слева).
  Текущий тайл генерит `tools/wall_topdown.py` (128px, бесшовно, не-разрушающе —
  пишет только wall_top.png). Идеальный апгрейд — AI-текстура по §7e.
...
### 7e. Стена — top-down блок со «встроенным» 3D (основной тайл стены)
```
Бесшовно тайлящаяся (seamless tileable) текстура СТЕНЫ для 2D top-down игры
(стиль The Escapists), вид строго СВЕРХУ, детальный пиксель-арт. Заполняет весь
квадратный кадр, без фона. Это ЕДИНСТВЕННЫЙ тайл стены — у стены нет отдельной
боковой грани, поэтому ОБЪЁМ должна давать сама текстура.
КОМПОЗИЦИЯ: бетонные/шлакоблочные блоки в перевязку (running bond, сдвиг рядов на
пол-блока), между блоками тёмный шов. Каждый блок выглядит как ПРИПОДНЯТЫЙ КУБ:
светлая фаска по ВЕРХНЕЙ и ЛЕВОЙ кромке (свет сверху-слева), тёмная фаска по
НИЖНЕЙ и ПРАВОЙ кромке (собственная тень). Единый источник света для всех блоков.
ЦВЕТ: холодный нейтрально-серый бетон, грань блока ~#4e5452, светлая фаска
~#787e7a, тёмная фаска/шов ~#282c2c, почти чёрные трещины. Без зелени; ржавчина —
редкий тёплый акцент.
