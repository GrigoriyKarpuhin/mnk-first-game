...
обзор. Визуально створка коротко уезжает в косяк: логическая проходимость
переключается сразу, а `PrisonDoor` за ~0.16 секунды интерполирует позицию и
ширину текущего door-спрайта до открытого или закрытого состояния. Ось открытия
выбирается по соседним стенам: стены слева/справа дают X-сдвиг, стены
сверху/снизу дают Y-сдвиг вверх.
...
Мини-карта больше не является обязательной частью HUD. Вместо неё игрок может
открыть полноэкранную карту на `M`. Карта показывает структуру крыла, подписи
основных помещений, положение героя и маркер активной цели из журнала. Стрелки
вверх/вниз меняют масштаб карты, стрелки влево/вправо сдвигают вид.

Обнаружение в стелсе происходит не по мини-карте, а по фактическим конусам
обзора охраны и камер в мире.
...
В момент успешного удушения надзиратель сразу перестаёт быть активным источником
обзора и поведения, но визуально остаётся стоять до завершения choke-анимации,
после чего отображается как лежащее тело.
...
Обычный удар игрока наносит надзирателю `1` урон. У обычного надзирателя `7`
очка здоровья: каждое попадание запускает погоню, а при нуле здоровья он
оглушается и становится телом, которое можно перенести или спрятать. Тихое
устранение со спины по `F` по-прежнему оглушает сразу.
Над охранником, получившим урон, во время этой погони показывается шкала
оставшегося здоровья; вне открытого боя она скрыта.
...
- **NPC:** программист, Ракель, другие заключённые, патрульный
  надзиратель.
...
- **Зачем приходит игрок:** продвигает route Ракель, подслушивает
...
- **Первое посещение:** подтверждает, что Ракель получает знания о тюрьме
  через наблюдение за персоналом и личные связи.
- **Открывается позже:** через route Ракель игрок может попасть туда с её
  помощью; позднее можно открыть альтернативный маршрут в блок C без её ключа.
- **Риск:** высокий, потому что это закрытая зона. При дружбе с Ракель
...
Требование к геометрии сада: это должна быть не маленькая открытая комната, где
любой патруль сразу видит весь объём, а увеличенная закрытая зона с внутренними
стенами из плотных кустов/живой изгороди. Кусты работают как стены для обзора и
движения, но визуально сохраняют ощущение искусственного сада. У входа из общего
блока нужен безопасный карман: Ракель и игрок после первой совместной сцены
оказываются в части сада, куда не заходит патруль и куда не смотрит камера.
Дальше игрок сам решает выйти из безопасной зоны в лабиринт, чтобы подслушать
персонал, обойти охранника или найти новый маршрут.
...
1. Игрок узнаёт через route Ракель, что старый вход в сад закрыли, а
   новый контролируется ключом.
2. При доверии Ракель может привести игрока в сад в безопасное окно.
...
| Вход в сад | Будущее расширение | Доверие Ракель или альтернативный маршрут | Route Ракель | Информационный узел, слухи персонала и путь к блоку C |
...
- Камера не должна дублировать широкий конус охранника: её обзор уже, короче и
  имеет слепую зону прямо под креплением и у боков рядом со стеной.
- Визуально камера показывает полупрозрачный красный луч, который плавно
  сканирует участок из стороны в сторону; фактическое обнаружение следует за
  этим лучом.
...
- Текущий доступ к тестовому крылу через дверь сада/блока C используется только
  чтобы route программиста можно было проверить независимо от route Ракель
  2. В финальном level design открытие сада не должно быть наградой программиста:
  сад остаётся зоной Ракель, слухов персонала и маршрутов побега.
- В целевом blockout отвёртка открывает ревизионную панель хозяйственной части,
  вентиляция остаётся единственным ранним входом в кухонный карман, а служебная
  дверь кухни впервые открывается из кухонной стороны и дальше становится
  двусторонним shortcut.
