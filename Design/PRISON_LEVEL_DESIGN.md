...
Статус: принятая схема первого игрового среза
...
Карта построена вокруг двух контрастных частей:

- публичный блок: камеры, общая зона, туалет и вход в эксперименты;
- служебный блок: кухня, служебный коридор, склад, комната персонала,
  лаборатория, инженерная зона и пустая комната.

Первый скрытый вход в служебный блок проходит из туалета через вентиляцию.
После получения служебного пропуска открываются два коротких пути:

- из служебного коридора к входу в эксперименты и общей зоне;
- из служебного коридора на кухню.
...
| Короткий путь в публичный блок | Служебный пропуск |
| Короткий путь на кухню | Служебный пропуск |
| Пустая комната | Глазной имплант |
...
- надзиратель служебного коридора проходит между входами в склад, комнату
  персонала, кухню и короткий путь;
- надзиратель инженерного крыла патрулирует подход к инженерной зоне,
  лаборатории и пустой комнате.

Надзиратели видят игрока по прямой перед собой. Стены, закрытые двери и массивные
...
|---|---|---|
| Самодельная отвёртка | Общая зона | Открывает вентиляционную решётку |
| Копия листа приёмки кухни | Комната персонала | Даёт доступ к кодовому замку склада |
| Служебный пропуск | Склад | Открывает инженерную зону и короткие пути |
| Глазной имплант | Инженерная зона | Открывает пустую комнату и позднее показывает камеры |
...
- **Соединена с:** общим блоком, вентиляцией к служебному коридору.
...
3. В туалете игрок замечает повреждённую вентиляцию и открывает решётку
   отвёрткой.
4. Через вентиляцию игрок впервые попадает в служебный коридор и наблюдает
   маршрут надзирателя.
